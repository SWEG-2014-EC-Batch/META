...
int main(){
	...
	ifstream fin;
		...
		fin.read((char *) & smn[i], sizeof(employee));
		for(i=0; i<4; i++){
		strcpy (any[i].name ,smn[i].name);
		
	if(smn[i].hours <= 40){
				
				any[i].overTimepay = 0;
				any[i].regularPay = smn[i].rate * smn[i].hours;}
	else if(smn[i].hours > 40){
			smn[i].overTime = smn[i].hours - 40;
			smn[i].hours = 40;
			any[i].regularPay = smn[i].rate * smn[i].hours;
			any[i].overTimepay = smn[i].rate * 1.5 * smn[i].overTime;
	}
